...
// S3 facade interface FROZEN per user "Must Remain Unchanged"
// Bucket facades: materials, avatars, snapshots, userassets, cdn, exports, useravatars, appassets, vendorassets
// SECURITY: File paths are constructed from server-controlled IDs (userId, materialId, assetId, content hash)
//           per AAP §0.5.2 Strategy H / R8 / OWASP A03 (path traversal defense)
//           User-supplied filenames are normalized via path-aware extraction before storage
...
function FileUtil() {
  ...
  this._upload = function(stream, container, s3, fileinfo, cb) {
    // SECURITY: aws-sdk patched within v2.x per AAP §0.5.3 (v3 migration deferred per Minimal Change Clause)
    //           v2 in maintenance mode but receiving security patches; v3 migration touches every S3 call site (high cost)
  ...
  this._fileToContainer = function(upload, container, s3, cb) {
    ...
    // SECURITY: Extension is extracted from user-supplied filename but only used as suffix on
    //           server-computed digest+fileId; not as a directory path component
    ...
    self.hashcontents(upload.path, function(digest) {
      // SECURITY: S3 Key constructed from server-computed SHA-1 digest + container.fileId + extension
      //           User-supplied filename is NEVER used directly as the S3 Key (path traversal defense)
      var fileinfo = {
        name        : digest,
  ...
  // SECURITY: SHA-1 used as deterministic content identifier hash for deduplication/uniqueness, NOT for confidentiality
  //           Acceptable per §6.4.4.1.1 / AAP §0.5.2 Strategy B / R2 / NIST SP 800-131A (SHA-1 allowed for non-cryptographic uses)
  //           Identity-only use: digest is concatenated with fileId/extension for S3 object key construction
  //           Integrity is NOT relied upon; collision risk is mitigated by accompanying authorization checks
  // returns sha1 digest of files contents
  this.hashcontents = function(path, cb) {
    var stream = fs.createReadStream(path);
    // SECURITY: SHA-1 identifier hash per annotation above (AAP §0.5.2 Strategy B / R2)
  ...
  this.uploadUserAvatar = function(upload, cb) {
    // SECURITY: MIME type validation prevents arbitrary file upload exploitation
    //           per AAP §0.5.2 Strategy H / R8 / OWASP A05 / CWE-434 Unrestricted Upload of File with Dangerous Type
    //           Whitelist regex enforces image-only uploads (png, jpg, jpeg only)
    //           Note: HTTP header content-type can be spoofed by client; defense-in-depth is provided by:
    //           (1) extension whitelist in config.app.extensionWhitelist (used by _fileToContainer),
    //           (2) S3 ContentType header set per validated input (no inline rendering on browser),
    //           (3) bucket policy restricting public access to image MIME types only
