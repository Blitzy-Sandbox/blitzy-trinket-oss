...
internals.findById = function(model, fallback) {
  return function(id, optional, next) {
    // The legacy Hapi 4 server-method signature was `(id, optional, next)`
    // where `optional` could be either a boolean controlling whether the
    // missing-id path returned a 404 (vs Boom.badRequest()) OR a fallback
    // document. The signature was also reused in callback style
    // `(id, next)` where the second arg is the callback.
    //
    // Hapi 20+ pre-handlers always call server methods through
    // `serverMethod.apply(null, args)` (see lib/util/routeParser.js
    // convertPreHandlers), and the routeParser does not append a `next`
    // callback. Therefore, when this function is invoked from a pre-handler
    // declaration like `parent(payload.parent,pre.lesson)`, we receive
    // `(undefined, lessonDoc)` and must treat the second argument as the
    // optional fallback — NOT as a callback. The original
    // `arguments.length === 2 && typeof optional !== 'boolean'` heuristic
    // mis-classified a Mongoose document as `next` and crashed with
    // "lessonDoc is not a function" (HTTP 500).
    if (typeof optional === 'function') {
      next = optional;
      optional = false;
    }
    // Otherwise: treat `optional` as the fallback document (or boolean flag).
    // No reassignment needed.

    if (!id) {
      // SECURITY/COMPAT: when called as a pre-handler with an `optional`
      // fallback document (legacy Hapi 4 idiom: `parent(payload.parent,
      // pre.lesson)`), missing id should RESOLVE with the fallback rather
      // than reject. The original code uniformly rejected, but the legacy
      // Hapi 4 callback contract `next(value)` accepted any non-Error value
      // as the success result, so `next(optional)` was actually a success
      // path in callback mode. Replicating that here for Promise mode keeps
      // moveMaterial's `parent(payload.parent,pre.lesson)` pre-handler
      // working when no payload.parent is supplied.
      if (optional && typeof optional === 'object' && !optional.isBoom) {
        return next ? next(optional) : Promise.resolve(optional);
      }
...
module.exports.verifyEmailToken = function(request, reply) {
  ...
  if (token) {
    data = jwt.verify(token, secret, { algorithms: ['HS256'] }); // SECURITY: pin algorithm to prevent algorithm confusion (CVE-2022-23540)
