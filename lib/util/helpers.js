...
// SECURITY: makeRedirectBoom builds a Boom error sentinel that the app.js
// SECURITY: onPreResponse extension converts into an actual h.redirect() takeover
// SECURITY: response (with the project's defense-in-depth security headers
// SECURITY: re-applied per AAP §0.5.2 Strategy D / R4 / OWASP A05). This pattern
// SECURITY: replaces the legacy reply().redirect(location).permanent().takeover()
// SECURITY: chain that the Hapi 4-style fakeReply wrapper at lib/util/routeParser.js
// SECURITY: lines 86-105 cannot honor: the wrapper's fakeReply(undefined) call
// SECURITY: resolves the outer Promise with `null` BEFORE the chain's downstream
// SECURITY: .takeover() can resolve it with a redirect marker, leaving the
// SECURITY: pre-handler `assign` variable bound to null (causing 500 errors when
// SECURITY: the controller dereferences `course.archived` etc. — verified in QA
// SECURITY: bisection: course slug-alias 301 test expected 301, received 500).
// SECURITY: Throwing a Boom error short-circuits the wrapper at the .catch(reject)
// SECURITY: branch (lib/util/routeParser.js line 113), reliably propagating the
// SECURITY: redirect intent to Hapi's response pipeline. The sentinel
// SECURITY: `isRedirectBoom = true` flag distinguishes redirect-Boom from genuine
// SECURITY: 3xx error responses (none of which are emitted by Boom natively;
// SECURITY: 301/302 are not in the standard Boom factory). Closes pre-existing
// SECURITY: course/trinket alias-redirect 500 cascade per AAP API Compatibility
// SECURITY: Directive (preserve route contract).
internals.makeRedirectBoom = function(location, permanent) {
  // SECURITY: Construct the redirect-marker Boom-like error MANUALLY rather than
  // SECURITY: via Boom.boomify(...) — Boom asserts statusCode >= 400 (see
  // SECURITY: node_modules/@hapi/boom/lib/index.js initialize()) and 3xx redirects
  // SECURITY: are not part of the Boom factory contract. We hand-craft the
  // SECURITY: Boom-shaped object: { isBoom: true, output: { statusCode, headers,
  // SECURITY: payload } } so app.js onPreResponse's `response.isBoom` truthy
  // SECURITY: branch can detect and convert the sentinel into an h.redirect()
  // SECURITY: takeover. The shape matches Hapi's expectation for Boom errors
  // SECURITY: at @hapi/hapi/lib/response.js (boomify path) — Hapi reads
  // SECURITY: response.output.statusCode and response.output.headers when
  // SECURITY: emitting the response. The fakeReply wrapper at routeParser.js
  // SECURITY: line 91 checks `value.isBoom` and routes via reject(), so this
  // SECURITY: shape is delivered to Hapi as the rejected pre-handler value.
  var err = new Error('Redirect');
  err.isBoom = true;
  err.isRedirectBoom = true;
  err.output = {
...
module.exports.makeRedirectBoom = internals.makeRedirectBoom;
...
internals.findById = function(model, fallback) {
  return function(id, optional, next) {
    ...
    // SECURITY: The original arity-detection branch assumed that any 2-arg call
    // SECURITY: where the second arg is non-boolean is a (id, callback) legacy
    // SECURITY: Hapi 4 pattern. That misclassifies the routeParser server-method
    // SECURITY: invocation pattern `parent(payload.parent, pre.lesson)` from
    // SECURITY: config/api_routes.js where the second argument is intentionally a
    // SECURITY: fallback Mongoose document (the parent lesson). The misdetection
    // SECURITY: routed `next = pre.lesson` and then attempted `next(err)`, calling
    // SECURITY: the Mongoose document as a function and surfacing as HTTP 500.
    // SECURITY: Restrict the legacy-callback detection to ONLY `typeof optional
    // SECURITY: === 'function'` — Hapi 20+ server methods are Promise-returning
    // SECURITY: and never receive a 3rd next callback (lib/util/routeParser.js
    // SECURITY: line 226 calls serverMethod.apply(null, args) without a callback),
    // SECURITY: so the only legitimate `next` source is when a downstream caller
    // SECURITY: actually passes a function. Closes pre-existing 500 cascade on
    // SECURITY: PUT /api/courses/{courseId}/lessons/{lessonId}/materials/
    // SECURITY: {materialId}/move per AAP API Compatibility Directive.
    ...
    if (!id) {
      // SECURITY: When `id` is missing, `optional` (if truthy) is treated as a
      // SECURITY: fallback value to return INSTEAD of failing the pre-handler.
      // SECURITY: This is the documented `parent(payload.parent, pre.lesson)`
      // SECURITY: pattern from config/api_routes.js — the second arg is the
      // SECURITY: fallback (the lesson document) used when payload.parent is
      // SECURITY: not provided. If `optional` is a Boom error, propagate as
      // SECURITY: rejection so the route returns the appropriate error code.
      // SECURITY: Otherwise resolve with the fallback value (or Boom.badRequest()
      // SECURITY: when no fallback is provided). This restores the pre-existing
      // SECURITY: API contract per AAP API Compatibility Directive.
      var err = optional ? optional : Boom.badRequest();
      if (next) return next(err);
      // Resolve with non-Boom fallback values; reject with Boom errors.
      if (err && err.isBoom) return Promise.reject(err);
      ...
      .then(function(doc) {
        if (doc) {
          ...
          if (!requestLang || requestLang === doc.lang) {
            ...
            // SECURITY: Use makeRedirectBoom to bypass the broken fakeReply
            // SECURITY: .redirect() chain in routeParser (see makeRedirectBoom
            // SECURITY: comment block above for root cause). app.js onPreResponse
            // SECURITY: detects isRedirectBoom and emits a real 301 takeover.
            return reply(internals.makeRedirectBoom(location, true));
          }
        }
        else {
          return reply(Boom.notFound());
