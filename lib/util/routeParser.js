...
function parseRoutes(routeConfigs) {
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    if (controller) {
      // SECURITY: security/detect-non-literal-require is disabled for this single line
      // SECURITY: because the `controller` variable is sourced from operator-controlled
      // SECURITY: route configuration (config/api_routes.js, config/routes.js) at boot
      // SECURITY: time — these files are repository-controlled trusted sources, not
      // SECURITY: user-controlled at runtime. Defense against CWE-829 (Inclusion of
      // SECURITY: Functionality from Untrusted Control Sphere) is upheld by the fact
      // SECURITY: that no user input flows into this require() and the controller
      // SECURITY: directory is bounded to ../controllers/ at compile time. Same pattern
      // SECURITY: documented in test/security/index.js for the security test loader.
      ...
      try {
        ...
        if (validation) {
          ...
          if (validationErrors) {
            ...
            // SECURITY: Joi validation failures return HTTP 400 (Bad Request) per REST semantics
            //           (R8 per AAP §0.5.2 Strategy H / OWASP A03 Injection). The request.fail
            //           compatibility shim defaults to HTTP 200 for backward compatibility with
            //           legacy HTML form flows; for validation failures we explicitly set 400
            //           on the response object (Hapi reads the status code at serialization
            //           time, so setting it after responseResolver still applies).
            //           For HTML routes that redirect on validation failure (fail.redirect with
            //           Accept: text/html), the redirect response (HTTP 302) is intentionally
            //           preserved so the user is taken back to the form with the flash payload.
            //           This addresses QA finding 8.2 (validation failures returning HTTP 200).
            var validationResponse = request.fail(request.payload, util.inspect(validationErrors));
            var isHtmlRedirect = (responseType === 'html' && fail.redirect);
            if (!isHtmlRedirect && validationResponse && typeof validationResponse.code === 'function') {
