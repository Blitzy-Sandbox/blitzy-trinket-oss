...
function convertPreHandlers(pre, server) {
  ...
  return pre.map(function(preHandler) {
      ...
      if (typeof preHandler.method === 'function') {
        ...
        var wrappedMethod = async function(request, h) {
          return new Promise(function(resolve, reject) {
            // SECURITY: defer resolve so the legacy chainable
            // `reply().redirect(url).permanent().takeover()` can override the
            // synchronous `reply()` resolution before the Promise settles.
            // The legacy Hapi 4 API allowed `reply()` (no args) followed by
            // a chained redirect; if we resolve(null) inside fakeReply then
            // the takeover()'s subsequent `resolve(redirectResponse)` is a
            // no-op (Promises are one-shot) and the pre-handler returns
            // null instead of the redirect — which leaves request.pre.<assign>
            // null and downstream handlers crash with "Cannot read
            // properties of null". Instead, capture `value` / redirect intent
            // synchronously and resolve once at the end of the microtask
            // queue, after the entire chain has executed.
            //
            // For the redirect path, build a real Hapi response via `h.redirect()`
            // so Hapi treats it as a redirect (sets Location header, status,
            // and honours .takeover()). The `permanent()` flag maps to status
            // 301 (vs default 302).
            var settled = false;
            var pendingValue = undefined;     // sentinel: reply not yet called
            var pendingRedirect = null;
            function settle() {
              if (settled) return;
              settled = true;
              if (pendingRedirect) {
                var resp = h.redirect(pendingRedirect.url);
                if (pendingRedirect._permanent) {
                  resp = resp.permanent();
                }
                if (pendingRedirect._takeover) {
                  resp = resp.takeover();
                }
                return resolve(resp);
              }
              if (pendingValue !== undefined) return resolve(pendingValue);
              resolve(null);
            }
            ...
            var fakeReply = function(value) {
              ...
              if (value !== undefined) {
                pendingValue = value;
                // Resolve immediately for the value-passing path; legacy
                // callers do `return reply(doc)` and rely on synchronous
                // resolution. The redirect chain is only entered when value
                // is undefined.
                settled = true;
                resolve(value);
              }
              ...
              return {
                redirect: function(url) {
                  pendingRedirect = { _isRedirect: true, url: url, _permanent: false, _takeover: false };
                  return {
                    permanent: function() { pendingRedirect._permanent = true; return this; },
                    takeover: function() { pendingRedirect._takeover = true; settle(); return this; }
                  };
                },
                takeover: function() { settle(); return this; }
              };
            };
            ...
            try {
              ...
              if (result && typeof result.then === 'function') {
                result.then(function(val) {
                  ...
                  // val === undefined: see if pendingRedirect was queued
                  settle();
                ...
                // Sync path: defer resolve to allow chain to complete
                setImmediate(settle);
              }
            } catch (err) {
    ...
    // Handle string format: 'isAdmin(user)'
      ...
      // Wrap old-style function(request, reply) to Hapi 20+ style
      // SECURITY: same deferred-resolve fix as above (see block-comment in
      // the object-form wrapper). Without this, legacy
      // `reply().redirect(url).permanent().takeover()` chains lose the
      // takeover resolution because the synchronous `resolve(null)` already
      // settled the Promise.
      var originalFunc = preHandler;
      var wrappedFunc = async function(request, h) {
        return new Promise(function(resolve, reject) {
          var settled = false;
          var pendingValue = undefined;
          var pendingRedirect = null;
          function settle() {
            if (settled) return;
            settled = true;
            if (pendingRedirect) {
              var resp = h.redirect(pendingRedirect.url);
              if (pendingRedirect._permanent) resp = resp.permanent();
              if (pendingRedirect._takeover)  resp = resp.takeover();
              return resolve(resp);
            }
            if (pendingValue !== undefined) return resolve(pendingValue);
            resolve(null);
          ...
          try {
            ...
            if (result && typeof result.then === 'function') {
              result.then(function(val) {
                if (settled) return;
                if (val !== undefined) { settled = true; return resolve(val); }
                settle();
              }).catch(function(err) { if (!settled) { settled = true; reject(err); } });
            } else if (result !== undefined && !settled) {
              settled = true;
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    route.handler = async function(request, h) {
      ...
      var responsePromise = new Promise(function(resolve) {
        responseResolver = function(value) {
          // SECURITY/COMPAT: idempotent settlement — once the response has been
          // captured, subsequent calls to reply()/request.success()/request.fail()
          // are no-ops. This protects against legacy promise chains that call
          // request.fail() then keep flowing into a downstream .then() that
          // would invoke request.success() with the previous response object as
          // its payload (which would recurse infinitely through serialize()).
          if (responseSettled) {
            return;
          }
      ...
      try {
        ...
        var reply = function(data) {
          // Idempotent: once the response is settled, drop subsequent calls.
          if (responseSettled) {
            return data;
          }
          if (data && data.isBoom) {
        ...
        request.success = function(json) {
          // Idempotent: once the response is settled (e.g. because request.fail()
          // already ran earlier in a promise chain that downstream .then() blocks
          // continued to flow through), drop this call. Without this guard the
          // serialize() walk in this function would recurse over a Hapi response
          // object that legacy controllers sometimes leak into the next .then(),
          // exhausting the stack. See QA-FINAL-2 finding "patchMaterialContent
          // 500 stack overflow".
          if (responseSettled) {
            return;
          }
