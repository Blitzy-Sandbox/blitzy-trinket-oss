...
var trinketConfig = {
  ...
  // SECURITY: python_types declaration mirrors public/js/trinket-markdown.js:11
  // SECURITY: (browser sibling). Empty array preserves the existing runtime behavior:
  // SECURITY: when python_types.indexOf(match[2]) returns -1 (always, for empty array),
  // SECURITY: the embed URL builder falls through to `match[2]` verbatim — identical to
  // SECURITY: the browser fallback path. The previously-undefined reference would have
  // SECURITY: thrown a ReferenceError when EMBED_URLS[3].url was invoked on a
  // SECURITY: trinket-host markdown link, breaking server-side markdown rendering for
  // SECURITY: course/trinket descriptions (CWE-754 Improper Check for Unusual Conditions).
  // SECURITY: Cross-version mirror parity per AAP §0.5.2 Strategy I / R9; fixes ESLint
  // SECURITY: no-undef error per AAP §0.5.2 Strategy I / R9 audit findings.
  var python_types    = [];
  ...
  module.exports = function(options) {
    function processCode(code, lang, escaped) {
      ...
      if (parts) {
        ...
        // SECURITY: highlight.js@^11 uses a new signature highlight(code, {language}); migrated from
        // the deprecated v9 highlight(lang, code) signature to remediate GHSA-7wwv-vh3v-89cq (ReDoS in
        // multiple grammars). v11 also drops auto-detection by default and ignores illegal sequences,
        // matching our explicit getLanguage() guard above.
        output = '<pre><code class="hljs">' + hljs.highlight(code, { language: lang, ignoreIllegals: true }).value + '</code></pre>';
      }
      else {
