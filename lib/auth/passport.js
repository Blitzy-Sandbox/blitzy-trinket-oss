      ...
      User.findByMultiple({ email : email, username : username, 'profiles.google.id' : profile.id }, function(err, user) {
        ...
        if (user) {
          ...
          user.save(function(err, newUser) {
              ...
              // SECURITY: input flows here) — empty object preserved as the minimal
              // SECURITY: backward-compatible value because (1) this entire configurePassport
              // SECURITY: branch is documented as DEAD CODE per the file header comment
              // SECURITY: (lines 11-37); the active runtime Google OAuth flow lives in
              // SECURITY: lib/controllers/auth.js googleCallback handler — the flash
              // SECURITY: payload has no consumer in lib/views/* templates so the empty
              // SECURITY: object preserves the original intent without information disclosure
              // SECURITY: (CWE-200). Fixes ESLint no-undef error per AAP §0.5.2 Strategy I / R9
              // SECURITY: while preserving dead-code defense-in-depth scaffolding for any
              // SECURITY: future Authentication.configure(Passport) wiring per the file header.
              var opts = {};
