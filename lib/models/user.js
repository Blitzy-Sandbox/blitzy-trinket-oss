...
function checkPermissions(next) {
  ...
  //
  // The role name "trinket-code" is preserved verbatim from the pre-open-
  // source-release Trinket role table; lib/models/roles.js exposes
  // permissions['trinket-code'] = permissions['user'], so the granted set
  // is identical to the simplified open-source 'user' role. Using the
  // legacy name keeps backwards compatibility with persisted DocumentDB
  // user records (which were written under the legacy name) and with the
  // roles plugin test suite (test/lib/models/plugins/roles.js) which
  // exercises hasRole('trinket-code') as the post-registration default.
  if (this.roles.length == 0) {
    this.setRoles("trinket-code", "site").then(next);
  }
  else {
    next();
  }
