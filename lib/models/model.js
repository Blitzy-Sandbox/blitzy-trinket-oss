    ...
    schema.methods['serialize'] = function() {
      ...
      for (var key in publicSpec) {
        if (Array.isArray(this[key])) {
          // Legacy contract: empty arrays are omitted from the serialized
          // payload. Tests assert `should.not.exist(lesson.materials)` and
          // `should.not.exist(course.lessons)` after delete operations, so
          // a Mongoose document with an emptied DocumentArray must not
          // surface that field at all. Non-empty arrays are serialized as
          // before so that downstream consumers (course outline, lesson
          // listings, etc.) keep their existing shape.
