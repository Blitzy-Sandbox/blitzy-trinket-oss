...
function hashify() {
  ...
  // SECURITY: SHA-1 used as deterministic identifier hash for trinket content hash (this.hash)
  // Acceptable per §6.4.4.1.1 / AAP §0.5.2 Strategy B / R2 — NOT a confidentiality, integrity, or
  // authentication primitive. NIST SP 800-131A allows SHA-1 for non-cryptographic identifier uses;
  // collision-vulnerability is acceptable because identifier security depends on accompanying
  // authorization checks in lib/controllers/trinket.js. URL surface backward compatibility requires
  // SHA-1 retention (changing the algorithm would invalidate all existing trinket hashes/shortCodes).
  this.hash = crypto.createHash('sha1').update(seed).digest('hex');

  if (!this.shortCode) {
    // SECURITY: SHA-1 used as deterministic identifier hash for trinket shortCode generation
    // (12-char URL identifier). Acceptable per §6.4.4.1.1 / AAP §0.5.2 Strategy B / R2 — NOT a
    // confidentiality, integrity, or authentication primitive. URL surface backward compatibility
    // requires SHA-1 retention (changing the algorithm would invalidate all existing shortCodes).
...
function verifyShortCode(timestamp) {
  ...
  // SECURITY: SHA-1 used in shortCode verification (deterministic comparison hash) — must match
  // the algorithm used in hashify() above. Acceptable per §6.4.4.1.1 / AAP §0.5.2 Strategy B / R2 —
  // NOT a confidentiality primitive. Verification compares recomputed shortCode against this.shortCode
  // for stale-shortCode detection; security depends on accompanying authorization checks in
  // lib/controllers/trinket.js, not on hash algorithm strength.
