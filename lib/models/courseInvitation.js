...
function addList(emailList, course) {
  ...
  return Promise.all(emailList.map(function(email) {
    // SECURITY: SHA-256 replaces MD5 to mitigate cryptographic weakness (OWASP A02 Cryptographic Failures)
    // per AAP §0.5.2 Strategy B / R2 / NIST SP 800-131A guidance against MD5 use.
    // .substring(0, 8) truncation preserved for invitation URL backward compatibility.
    // Token security relies on accompanying authorization checks per §6.4.4.1.1; SHA-256 closes the
    // collision-vulnerability surface even though invitation tokens are not used as a confidentiality primitive.
    token = crypto.createHash("sha256").update(email + course.id).digest("hex").substring(0, 8);
