var config       = require('config'),
    ...
    // remediate CVE-2023-28155 (cross-protocol redirect SSRF in the Request
    // package; the package is no longer supported by the maintainer). The
    // single call-site in assetUploadFromURL has been adapted to axios's
    // stream-response API while preserving the same write-then-upload
    // sequence. AAP §0.5.1 / QA finding #4.
    ...
    // SECURITY: replaced deprecated `node-uuid` with the maintained `uuid`
    // package (CVE-2015-8851 in node-uuid <1.4.4; node-uuid is deprecated and
    // its successor is `uuid`). The CommonJS export shape is identical, so
    // any future call to `uuid.v4()` continues to work. AAP §0.5.1 / QA #5.
    uuid         = require('uuid'),
...
module.exports = {
  ...
  assetUploadFromURL : function(request, reply) {
    ...
    tmp.tmpName(function(err, tmpPath) {
      ...
      // SECURITY: axios streaming download replaces the previous `request`
      // chained-event API. The download/upload sequence is preserved: we
      // record the upstream Content-Type from the response headers, pipe the
      // stream to `tmpPath`, and on the writable stream's `finish` event we
      // hand the file to FileUtil.uploadUserAsset — exactly the same control
      // flow that the original `.on('end', ...)` callback implemented.
      // AAP §0.5.1 / QA finding #4.
      _request.get(request.payload.url, { responseType: 'stream' })
