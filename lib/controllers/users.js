...
module.exports = {
  // SECURITY: welcome handler renders a server-side HTML response listing copy links for
  // SECURITY: courses owned by the configured trinket library user (config.app.trinketLibraryUser).
  // SECURITY: -----------------------------------------------------------------------------------
  ...
  // SECURITY: welcome-page UX (per test/lib/api/registration.js line 72-77) by enumerating the
  ...
  // SECURITY: -----------------------------------------------------------------------------------
  // SECURITY: Output safety per OWASP A03 Injection prevention:
  ...
  // SECURITY:     maintain post-signup banner UX rendered by lib/views/base.html.
  ...
  welcome : function(request, reply) {
    ...
    User.findByLogin(config.app.trinketLibraryUser, function(err, libraryUser) {
      ...
      Course.findForUser(libraryUser.id, function(err2, courses) {
        ...
        // SECURITY: app.js onPreResponse extension.
  ...
  assetUploadFromURL : function(request, reply) {
    ...
    tmp.tmpName(function(err, tmpPath) {
      ...
      .then(function(response) {
        ...
        // SECURITY: source-stream error handler — propagates remote-side
        // errors (mid-stream connection drop, TLS failure, server reset)
        // into the controller so a 2xx-then-failure no longer silently
        // stalls the writeStream. axios's `responseType: 'stream'` returns
        // a Node.js Readable whose `pipe()` does NOT forward errors by
        // default; the pre-AAP `request` library propagated source-stream
        // errors through its own callback, so this listener restores
        // application-level parity with that contract. Resolves
        // SECURITY.md Residual-Risk R-16.
        response.data.on('error', function(err) {
          console.log('source stream error:', err);
          // Tear down the writeStream so the partial tmp file is not held
          // open; ignore secondary errors from destroy() — they would only
          // be raised if the underlying fd is already closed.
          try { writeStream.destroy(err); } catch (e) {}
        });
      })
      .catch(function(err) {
  ...
  requestExport : function(request, reply) {
    ...
    Export.findPendingOrProcessing(userId)
      .then(function(existingExport) {
        if (existingExport) {
          // SECURITY: stringify ObjectId before passing through response shim.
          // routeParser.js's request.fail/request.success shim invokes
          // ObjectUtils.serialize() (lib/util/objectUtils.js:34) which walks
          // unknown object types via `for (var key in json)`. A raw Mongoose
          // ObjectId has internal `_bsontype`/`id` enumerable properties
          // that produce malformed serialized output, breaking Hapi 20+'s
          // headers.js content-type computation and causing the response
          // to surface as HTTP 500 even though the underlying business
          // logic succeeded. Calling .toString() yields the canonical
          // 24-char hex string that serialize() handles cleanly.
          request.fail({
      ...
      .then(function(saved) {
        ...
        // SECURITY: stringify ObjectId before passing through response shim
        // (see comment on the in-flight check above; the same serialization
        // hazard applies to the success path — line 1087 already calls
        // .toString() correctly on the queue payload, but the response
        // payload was missing it, causing POST /api/exports to surface as
        // HTTP 500 despite the export record being persisted successfully).
        return request.success({
          success: true,
          data: {
            exportId: exportRecord._id.toString(),
