var config       = require('config'),
    ...
    // request was removed from package.json. The audit/uniformity changes scoped to this file
    // (downloadExport ownership, signup error message uniformity, sendPassReset enumeration
    // defense per AAP §0.6.1) are layered on top by the users.js remediation agent.
...
module.exports = {
  ...
  assetUploadFromURL : function(request, reply) {
    ...
    tmp.tmpName(function(err, tmpPath) {
      ...
      // SECURITY: axios stream replaces request streaming pattern; preserves identical observable
      // behavior — record content-type from upstream response, pipe to tmp file, then upload.
