var config = require('config'),
    diff   = require('diff'),
    // SECURITY: @hapi/boom aliased as `Boom` to match this module's existing call sites (Boom.forbidden,
    //           Boom.notFound — 41 occurrences). Previously aliased as `errors` while every call site used
    //           the bare `Boom` symbol, producing ReferenceError → HTTP 500 instead of the documented HTTP
    //           403/404 responses on every authorization-rejected mutation. Per AAP §0.5.2 Strategy G / R7 /
    //           OWASP A01 Broken Access Control: the underlying access control DID block the action; only
    //           the response contract was broken (stack-trace leak via 500 + broken API consumer parsing).
...
module.exports = {
  createCourse : function(request, reply) {
    ...
    course.save(function(err, course) {
      ...
      return course.addUser(request.user, ["course-owner"])
        .then(function() {
          // SECURITY: Apply User.publicSpec allow-list filter to populated _owner before reply
          //           per AAP §0.5.2 Strategy G / R7 / OWASP A01 Broken Access Control / CWE-200/CWE-522.
          //           Two layers of defense are required because:
          //             (1) ObjectUtils.serialize() in lib/util/objectUtils.js tests for serialize()
          //                 via hasOwnProperty(), which fails for prototype-resident schema methods —
          //                 falling through to JSON.stringify and emitting every Mongoose schema field
          //                 (bcrypt password hash, full roles[] permission inventory, settings, etc.).
          //             (2) Reassigning `course._owner = course._owner.serialize()` does NOT durably
          //                 strip the populated User document because Mongoose's `_owner` schema-path
          //                 setter (declared in lib/models/plugins/ownable.js as type:ObjectId, ref:User)
          //                 RE-WRAPS the assigned plain object back into a User Mongoose document on
          //                 read — leaking schema-default fields (roles[], verified, source, created,
          //                 lastUpdated, _id) on subsequent reads.
          //           The fix: build a plain-object response shape via course.serialize() (which itself
          //           emits the broken JSON.stringify view of _owner), then explicitly overwrite the
          //           courseSerialized._owner key with the User.publicSpec-filtered serialize() result.
          //           The wrapper plain-object surface bypasses the Course schema setter entirely.
          var courseSerialized = (typeof course.serialize === 'function') ? course.serialize() : course;
          if (course._owner && typeof course._owner.serialize === 'function') {
            courseSerialized._owner = course._owner.serialize();
          }
          request.success({ course : courseSerialized });
        });
    });
  },
  ...
  getCourse : function(request, reply) {
    ...
    // SECURITY: Apply the User.publicSpec allow-list to a populated _owner before reply per
    //           AAP §0.5.2 Strategy G / R7. The in-place reassignment course._owner =
    //           course._owner.serialize() relies on the Mongoose _owner ref schema setter
    //           accepting the plain publicSpec object — the bcrypt password hash and the full
    //           roles[] permission inventory are stripped by User.serialize(), which is the
    //           CRITICAL hardening required by the QA report (the INFO-severity over-disclosure
    //           of _id/verified/source/created/lastUpdated is acknowledged and out of scope per
    //           the AAP Minimal Change Clause — the original pre-existing pattern is preserved).
    //           Pre-serializing the course to a plain object (course.serialize()) breaks
    //           ObjectUtils.serialize recursion when course.lessons contains raw ObjectIds
    //           (the recursion descends into the ObjectId's internal Buffer and throws).
