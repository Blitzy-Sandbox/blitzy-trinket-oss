...
// SECURITY: when diff was upgraded from ~1.0.8 to ^5.2.2 to address ReDoS
// (GHSA-h6ch-v84p-w6p9) and parsePatch DoS (GHSA-73rr-hh4g-fpgx), the
// strict patcher no longer accepts patches whose context lines do not
// match the source — diff 1.0.8 silently emitted the concatenation of
// context and addition lines when the source was empty, and the trinket
// editor relies on that legacy semantics for the "create initial content
// from a patch" workflow used by the lesson-content authoring UI.
// applyPatchCompat preserves diff 1.0.8 behavior for the empty-source
// edge case (the only case where the upgrade changed observable behavior)
// while delegating to the secure modern patcher in every other case.
function applyPatchCompat(source, patchText) {
  var result = diff.applyPatch(source, patchText);
  if (result !== false) {
    return result;
  }
  ...
  var parsed;
  try {
    parsed = diff.parsePatch(patchText);
  } catch (e) {
    return false;
  }
  ...
  for (var i = 0; i < parsed.length; i++) {
    var hunks = parsed[i].hunks || [];
    for (var j = 0; j < hunks.length; j++) {
      var lines = hunks[j].lines || [];
      var delims = hunks[j].linedelimiters || [];
      for (var k = 0; k < lines.length; k++) {
        var line = lines[k];
        var delim = delims[k] !== undefined ? delims[k] : '\n';
        var leader = line.charAt(0);
        if (leader === ' ' || leader === '+') {
          pieces.push(line.substring(1) + delim);
        }
      }
    }
  }

  return pieces.length > 0 ? pieces.join('') : false;
}
      ...
      return trinketPromise
        .then(function(trinket) {
          ...
          if (typeof(request.payload.patch) !== 'undefined') {
            var patched = applyPatchCompat(material.content ? material.content : '', request.payload.patch);
            if (patched === false) {
              return request.fail({
                data : _.extendOwn({
                    status  : "error"
