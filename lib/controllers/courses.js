...
module.exports = {
  ...
  download : function(request, reply) {
    ...
    ||  request.user.hasPermission("make-course-copy", "course", { id : course.id })) {
      ...
      // SECURITY: mkdirp@3 returns a Promise natively (no util.promisify wrapper needed).
      // mkdirp@3 CJS module.exports is an *object*, not a callable function — the async
      // Promise-returning function is exposed as the named .mkdirp property. Bare
      // `var mkdirpify = mkdirp; mkdirpify(path)` throws "TypeError: mkdirpify is not a
      // function" at runtime; bare `util.promisify(mkdirp)` (the pre-remediation form)
      // hangs because util.promisify wraps a function that already returns a Promise.
      // The .mkdirp named-export accessor below is the correct CJS interop adapter and
      // preserves the existing `mkdirpify(path).then(...)` call shape used by the
      // `mkLessonDirs` per-lesson directory creation and the top-level `courseDir`
      // creation in this `download` handler.
      var mkdirpify = mkdirp.mkdirp;
