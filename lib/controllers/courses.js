var _           = require('underscore'),
    ...
    // SECURITY: @hapi/boom aliased as `Boom` to match this module's exportCourse() Boom.forbidden() call
    //           site (line 289); pre-existing alias `errors` was unused by any call site and produced
    //           ReferenceError → HTTP 500 instead of HTTP 403 on cross-user export attempts. Per
    //           AAP §0.5.2 Strategy G / R7 / OWASP A01 Broken Access Control / CWE-639 IDOR.
    ...
    return Course.findFeaturedForUser(request.user)
      .then(function(courses) {
        courses = _.map(courses, function(course) {
          // SECURITY: Explicit `var page` declaration prevents accidental global
          // SECURITY: variable creation (CWE-471 / CWE-1023). Without `var`, repeated
          // SECURITY: invocations during _.map iteration would mutate a global `page`
          // SECURITY: across requests in the Node.js single-process model, leaking
          // SECURITY: course page data between concurrent requests (CWE-200 Information
          // SECURITY: Exposure). Fixes ESLint no-undef error per AAP §0.5.2 Strategy I /
          // SECURITY: R9 audit findings; mirrors the parallel fix in
          // SECURITY: lib/controllers/classes.js findFeaturedForUser fallback handler.
          var page    = course.page;
