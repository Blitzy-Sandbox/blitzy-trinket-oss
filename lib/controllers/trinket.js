  ...
  create : function(request, reply) {
    ...
    return trinket.save()
      .then(function(doc) {
        ...
        emailToken  = jwt.sign({ shortCode: doc.shortCode }, emailSecret, { algorithm: 'HS256', expiresIn: '7d' }); // SECURITY: explicit algorithm + expiry for email-share JWT
  ...
  createFork : function(request, reply) {
    ...
    return Trinket.findByIdAndUpdateMetrics(parent.id, 'forks', meta)
      ...
      .then(function(savedTrinket) {
        ...
        emailToken  = jwt.sign({ shortCode: savedTrinket.shortCode }, emailSecret, { algorithm: 'HS256', expiresIn: '7d' }); // SECURITY: explicit algorithm + expiry for email-share JWT
    ...
    return promise
      .then(function(result) {
        ...
        if (trinket.shortCode) {
          ...
          emailToken  = jwt.sign({ shortCode: trinket.shortCode }, emailSecret, { algorithm: 'HS256', expiresIn: '7d' }); // SECURITY: explicit algorithm + expiry for email-share JWT
          if (library) {
      ...
      .then(function(trinket) {
        ...
        try {
          ...
          if (/blocks/.test(trinket.lang)) {
            mainName = "main.xml";
            type     = mime.getType(mainName) || type; // SECURITY: mime@3 API rename (was mime.lookup); mitigates CVE-2017-16138
          }
          else {
            mainName = "main.txt";
          }
    ...
    return Trinket.findById(request.params.shortCode)
      .then(function(trinket) {
        ...
        if (file) {
          ...
          type     = mime.getType(file) || type; // SECURITY: mime@3 API rename (was mime.lookup); mitigates CVE-2017-16138
