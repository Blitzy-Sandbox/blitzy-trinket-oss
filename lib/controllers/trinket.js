// SECURITY: JWT email-share tokens use HS256 algorithm with explicit pinning per AAP §0.5.2 Strategy B / R2
//           per CVE-2022-23529 mitigation (jsonwebtoken v9+ requires explicit algorithm parameter)
//           Token signing secret = config.app.mail.secret + trinket.shortCode
//           Boot guard in app.js validates app.mail.secret >= 32 chars (parallel to session password guard)
//           verifyEmailToken in lib/util/helpers.js (line 252) consumes the same secret with algorithms: ['HS256']
//
// SECURITY: Mongoose query construction uses schema-typed ObjectId/String fields per AAP §0.5.2 Strategy H / R8
//           Trinket._id, _owner, _creator, _parent are ObjectId-typed; user-supplied values coerced via
//           explicit `mongoose.Types.ObjectId(...)` or implicit Mongoose schema casting on findById/findOne
//           Joi schemas in config/api_routes.js further reject operator-prefixed keys ($where, $regex, $gt, etc.)
//           per AAP §0.5.2 Strategy H / R8 / OWASP A03 NoSQL Injection
...
// SECURITY: Mongoose query construction audit per AAP §0.5.2 Strategy H / R8 / OWASP A03 NoSQL Injection
//           - Trinket.findById(id) / .findOne({_id: id}) / .findByIdAndUpdateMetrics(id, ...) :
//             Mongoose schema typing on Trinket._id (ObjectId) auto-coerces user-supplied values;
//             malformed/operator-prefixed values throw CastError and reject the request
//           - Trinket.findRemix(parentId, userId) : Mongoose schema typing on _parent and _owner ObjectIds
//           - Aggregation pipelines with $match { _owner, _id, folder.folderId } : ObjectId-typed schema
//             paths protect against operator injection; explicit `new ObjectId(query.folder)` coercion
//             on user-supplied folder query parameter (see list handler for reference)
//           - Joi schemas in config/api_routes.js (Joi.string(), Joi.object()) further reject operator-prefixed
//             keys at the validation layer per AAP §0.5.2 Strategy H / R8
//           - shortCode (String type) provides type-coercion against {$gt: ''} or {$where: '...'} payloads
module.exports = {
  ...
  list : function(request, reply) {
    ...
    // SECURITY: Aggregation pipeline filter scoped to current user's _id and non-deleted trinkets
    //           per AAP §0.5.2 Strategy G / R7 / OWASP A01 Broken Access Control
    //           request.user._id is server-controlled (set via Passport deserializeUser)
    //           Mongoose schema typing on Trinket._owner (ObjectId) provides defense against NoSQL operator injection
    ...
    if (request.query.folder) {
      // SECURITY: Explicit ObjectId coercion of user-controlled query parameter prevents NoSQL operator injection
      //           per AAP §0.5.2 Strategy H / R8 / OWASP A03
      //           Invalid ObjectId values throw CastError and reject the request
    ...
    // SECURITY: Admin-only override to query trinkets owned by another user per AAP §0.5.2 Strategy G / R7 / OWASP A01
    //           Gated by request.user.hasRole("admin") check; non-admin users cannot supply ?user= override
    //           Mongoose User.findById applies schema typing on User._id (ObjectId) per AAP §0.5.2 Strategy H / R8
  ...
  create : function(request, reply) {
    ...
    return trinket.save()
      .then(function(doc) {
        // SECURITY: HS256 secret validated at boot for minimum 32-char entropy per AAP §0.5.2 Strategy B / R2
        //           Boot guard in app.js refuses startup if app.mail.secret is missing or too short
        //           algorithm: 'HS256' explicit pinning prevents CVE-2022-23529 algorithm confusion (jsonwebtoken v9+)
        emailSecret = config.app.mail.secret + doc.shortCode;
        emailToken  = jwt.sign({ shortCode: doc.shortCode }, emailSecret, { algorithm: 'HS256' });
  ...
  createFork : function(request, reply) {
    ...
    return Trinket.findByIdAndUpdateMetrics(parent.id, 'forks', meta)
      ...
      .then(function(savedTrinket) {
        ...
        emailToken  = jwt.sign({ shortCode: savedTrinket.shortCode }, emailSecret, { algorithm: 'HS256' });
    ...
    return promise
      .then(function(result) {
        ...
        if (trinket.shortCode) {
          ...
          emailToken  = jwt.sign({ shortCode: trinket.shortCode }, emailSecret, { algorithm: 'HS256' });
          if (library) {
  ...
  snapshot : function(request, reply) {
    if (request.payload.snapshotData) {
      ...
      // SECURITY: Buffer.from replaces deprecated/unsafe new Buffer(arg, 'base64') (CWE-665 / Node.js
      // SECURITY: deprecation DEP0005). The unsafe constructor could allocate uninitialized memory
      // SECURITY: when called with a number argument, leaking previously freed memory contents into
      // SECURITY: the response payload. Buffer.from explicitly decodes base64 input as a string and
      // SECURITY: never zero-fills (per ESLint security/detect-new-buffer); fixes ESLint security
      // SECURITY: error per AAP §0.5.2 Strategy I / R9 audit findings.
