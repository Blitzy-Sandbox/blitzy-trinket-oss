var _             = require('underscore'),
    ...
    _request      = require('axios'), // SECURITY: replaced deprecated request with axios (CVE-2023-28155)
...
module.exports = {
  ...
  googleCallback : function(request, h) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      // SECURITY: explicit { timeout: 10000 } per SECURITY.md Residual-Risk
      // R-17 — axios's default of 0 means "no timeout", so a slow or
      // unresponsive Google OAuth endpoint could hang the request handler
      // until Hapi's outer request timeout fires. A 10-second per-call
      // timeout fails fast and lets the .catch() below report a clean
      // error to the user.
      _request.post(
    ...
    .then(function(accessToken) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        // SECURITY: explicit { timeout: 10000 } per SECURITY.md Residual-Risk
        // R-17 — axios's default of 0 means "no timeout", so a slow or
        // unresponsive Google profile endpoint could hang the request
        // handler until Hapi's outer request timeout fires. A 10-second
        // per-call timeout fails fast and lets the .catch() below report a
        // clean error to the user.
        _request.get('https://www.googleapis.com/oauth2/v2/userinfo', {
