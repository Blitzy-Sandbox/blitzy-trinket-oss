...
| R-17 | Outbound axios calls in `lib/controllers/auth.js` (Google OAuth token exchange POST and Google profile GET) have no explicit `timeout` option configured. axios's default is `0` (no timeout). **REMEDIATED in this audit pass: `{ timeout: 10000 }` added to both axios POST/GET option objects in `lib/controllers/auth.js`** (see inline `// SECURITY:` annotations citing R-17). A 10-second per-call timeout fails fast and lets the existing `.catch()` report a clean error to the user, avoiding a hang until Hapi's outer request timeout fires. | Resolved   | Per-call axios timeouts added                                 | n/a &mdash; closed.                                                                                                          |
